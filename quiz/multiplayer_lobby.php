...
<body>
    <div style="position: absolute; top: 20px; left: 20px;">
        <a href="multiplayer_exit.php?room=<?php echo $roomCode; ?>" class="btn"
    ...
    <div class="container" style="text-align: center; margin-top: 60px;">
        ...
        <?php if ($isHost): ?>
            <div style="margin-top: 30px; display: flex; gap: 10px; justify-content: center;">
                <button type="button" onclick="openInviteModal()" class="btn"
                    style="font-size: 1.2em; padding: 15px 30px; background: var(--accent-blue);">
                    <i class="fas fa-user-plus"></i> UNDANG TEMAN
                </button>

                <form method="POST">
                    <button type="submit" name="start_game" class="btn"
                        style="font-size: 1.2em; padding: 15px 40px; background: #2ecc71;">
    ...
    <script>
        ...
        function checkLobby() {
            fetch(`../api/api_multiplayer.php?action=check_lobby&room=${roomCode}`)
                .then(res => res.json())
                .then(data => {
                    // Update Player List
        ...
        function openInviteModal() {
            Swal.fire({
                ...
                didOpen: () => {
                    fetch('../api/api_invite.php?action=list_friends')
                        .then(res => res.json())
                        .then(friends => {
        ...
        function sendInvite(username) {
            ...
            fetch('../api/api_invite.php?action=send_invite', {
                method: 'POST',
                body: formData
            })
                .then(res => res.json())
