...
<head>
    ...
    <style>
        ...
        .btn-add { background: var(--accent-teal); color: white; }
    ...
    <div class="container">
        ...
        <div class="materi-card">
            ...
            <form action="friends.php" method="GET" class="search-box">
                <input type="text" name="q" class="search-input" placeholder="Cari username atau nama..." value="<?php echo htmlspecialchars($searchQuery); ?>">
                    ...
                    <div class="friend-grid">
                        <?php foreach ($searchResults as $res): ?>
                            <div class="friend-card">
                                <img src="<?php echo !empty($res['profile_pic']) ? (strpos($res['profile_pic'], 'http')===0 ? $res['profile_pic'] : 'img/'.$res['profile_pic']) : 'img/default-pp.png'; ?>" class="friend-img">
                                <div class="friend-info">
                                    <a href="profile.php?user=<?php echo $res['username']; ?>" class="friend-name"><?php echo $res['full_name'] ?? $res['username']; ?></a>
                                    <div class="friend-username">@<?php echo $res['username']; ?></div>
                                </div>
                                <div>
                                    <?php if ($res['friend_status'] === 'none'): ?>
                                        <button onclick="friendAction('add', '<?php echo $res['username']; ?>')" class="action-btn btn-add"><i class="fas fa-user-plus"></i> Add</button>
                                    <?php elseif ($res['friend_status'] === 'sent'): ?>
                                        <button onclick="friendAction('cancel', '<?php echo $res['username']; ?>')" class="action-btn btn-secondary"><i class="fas fa-times"></i> Batal</button>
                                    <?php elseif ($res['friend_status'] === 'received'): ?>
                                        <button onclick="friendAction('accept', '<?php echo $res['username']; ?>')" class="action-btn btn-accept"><i class="fas fa-check"></i> Terima</button>
                                    <?php elseif ($res['friend_status'] === 'friend'): ?>
                                        <span class="badge" style="background:var(--accent-teal); padding:5px 10px; border-radius:5px; color:white;"><i class="fas fa-check-circle"></i> Teman</span>
                                    <?php endif; ?>
                                </div>
                            </div>
                        <?php endforeach; ?>
            ...
            <div>
                ...
                <?php else: ?>
                    <?php foreach ($friends as $f): ?>
                        <div class="friend-card">
                            <img src="<?php echo !empty($f['profile_pic']) ? (strpos($f['profile_pic'], 'http')===0 ? $f['profile_pic'] : 'img/'.$f['profile_pic']) : 'img/default-pp.png'; ?>" class="friend-img">
                            <div class="friend-info">
                                <a href="profile.php?user=<?php echo $f['username']; ?>" class="friend-name"><?php echo $f['full_name']; ?></a>
                                <div class="friend-username">@<?php echo $f['username']; ?></div>
                            </div>
                            <button onclick="confirmRemove('<?php echo $f['username']; ?>')" class="action-btn btn-reject" title="Hapus Teman"><i class="fas fa-user-minus"></i></button>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
            </div>
            ...
            <div>
                ...
                <?php else: ?>
                    <?php while ($in = mysqli_fetch_assoc($incomingResult)): ?>
                        <div class="friend-card">
                            <img src="<?php echo !empty($in['profile_pic']) ? (strpos($in['profile_pic'], 'http')===0 ? $in['profile_pic'] : 'img/'.$in['profile_pic']) : 'img/default-pp.png'; ?>" class="friend-img">
                            <div class="friend-info">
                                <a href="profile.php?user=<?php echo $in['requester']; ?>" class="friend-name"><?php echo $in['full_name'] ?? $in['requester']; ?></a>
                                <div class="friend-username">@<?php echo $in['requester']; ?></div>
                            </div>
                            <div style="display:flex; gap:5px;">
                                <button onclick="friendAction('accept', '<?php echo $in['requester']; ?>')" class="action-btn btn-accept"><i class="fas fa-check"></i></button>
                                <button onclick="friendAction('reject', '<?php echo $in['requester']; ?>')" class="action-btn btn-reject"><i class="fas fa-times"></i></button>
                            </div>
                        </div>
                    <?php endwhile; ?>
                <?php endif; ?>
                ...
                <?php if (mysqli_num_rows($outgoingResult) > 0): ?>
                    ...
                    <?php while ($out = mysqli_fetch_assoc($outgoingResult)): ?>
                        <div class="friend-card" style="opacity: 0.8;">
                            <img src="<?php echo !empty($out['profile_pic']) ? (strpos($out['profile_pic'], 'http')===0 ? $out['profile_pic'] : 'img/'.$out['profile_pic']) : 'img/default-pp.png'; ?>" class="friend-img">
                            <div class="friend-info">
                                <span class="friend-name"><?php echo $out['full_name'] ?? $out['receiver']; ?></span>
                                <div class="friend-username">Menunggu konfirmasi...</div>
                            </div>
                            <button onclick="friendAction('cancel', '<?php echo $out['receiver']; ?>')" class="action-btn btn-secondary"><i class="fas fa-times"></i></button>
                        </div>
                    <?php endwhile; ?>
                <?php endif; ?>
            </div>
